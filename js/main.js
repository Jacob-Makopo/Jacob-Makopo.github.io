/* ============================================================== */
/* Template Name : Zowitch - Creative Personal Page               */
	...
	zowitch.processContactForm = function() {
		...
		form.on('submit', function(e) {
			...
			if (!name || !email || !subject || !message) {
				const response = 'Please complete the form and try again.';
				failFunc(response);
				return;
			}

			if (!/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(email)) {
				const response = 'Please enter a valid email address.';
				failFunc(response);
				return;
			}

			const response = 'Thank you! Your message has been sent.';
			doneFunc(response);
		});
	};
